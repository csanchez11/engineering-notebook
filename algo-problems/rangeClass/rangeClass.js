...
var Range = function(start = null, end = null, step = 1) {
  this.start = start;
  this.end = end;
  this.step = step;
};
...
Range.prototype.size = function () {
  if (!this.start) {
    ...
    if (this.end > this.start) {
      return Math.ceil(((this.end - this.start) + 1) / Math.abs(this.step))
    } else if (this.start > this.end) {
      return Math.ceil(((this.start - this.end) + 1) / Math.abs(this.step))
    }
  } else {
    return 1;
  }
  //need to account for end lower than start and neg step
};
...
//Need to edit includes to account for no end.
